...
- **Plugin-settings screenshot capture.** Automatically open each *enabled* plugin on a track in the **`mixprep_copy`**, screen-capture its GUI (`screencapture`), and file the shots into an easily recalled per-track folder (outside the repo, C-3), recording each shot's path into the manifest's existing `ref` / `channel_strip_ref` pointer fields. This is the automated, richer counterpart to the Phase 1 manual plugin-settings log; screenshots become the primary *visual* recall artifact while the structured `params`/`settings` remain useful for searchable, precise values and for plugins that must be rebuilt in Ableton. **Feasibility caveat:** because Logic exposes no real scripting API, enumerating/opening plugin windows requires fragile macOS UI/accessibility scripting and is version-sensitive; more reliable alternatives (saving Channel Strip Settings `.cst`, exporting per-plugin AU presets) should be offered alongside. Runs only against the copy (C-1/C-2/C-9); the repo stores only pointers (C-3/C-7). (Final placement between Phase 2 and Phase 3 and the concrete mechanism are under design — see Open Questions.)
...
- **Plugin-settings screenshot capture (Phase 2/3).** Auto-open each enabled plugin and screenshot its settings into a recallable folder, populating the `ref`/`channel_strip_ref` pointers. Open: exact phase placement (2 vs 3), how reliable Logic UI-scripting + `screencapture` actually is, whether to prefer/offer more reliable alternatives (`.cst` Channel Strip Settings, `.aupreset` export), the shots folder/naming layout, and the CLI surface (e.g. a later `mixprep shots` command + LLM-harness orchestration). Under active design.
